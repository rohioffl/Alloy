...
for arg in "$@"; do
  case "$arg" in
    ...
    -uptime-kuma-url=*) UPTIME_KUMA_URL="${arg#*=}" ;;
    -uptime-kuma-key=*) UPTIME_KUMA_API_KEY="${arg#*=}" ;;
...
# ---- Step 2d: optional Uptime Kuma source ------------------------------------
if [ -n "$UPTIME_KUMA_URL" ]; then
  if [ ! -f /etc/prometheus/prometheus.yml ]; then
    warn "Prometheus config not found — cannot add Uptime Kuma source"
  else
    log "Adding Uptime Kuma Prometheus source..."
...
PY
    ...
    if [ -z "$KUMA_TARGET" ]; then
      warn "Invalid -uptime-kuma-url value: ${UPTIME_KUMA_URL}"
    else
      KUMA_AUTH_BLOCK=""
    ...
    basic_auth:
      ...
      TMP_PROM=$(mktemp)
      cp /etc/prometheus/prometheus.yml "$TMP_PROM"
...
PY
      if command -v promtool >/dev/null 2>&1 && ! promtool check config "$TMP_PROM" >/dev/null 2>&1; then
        rm -f "$TMP_PROM"
        warn "Uptime Kuma source was not added because Prometheus config validation failed"
      else
        cat "$TMP_PROM" > /etc/prometheus/prometheus.yml
        rm -f "$TMP_PROM"
        curl -s -X POST http://127.0.0.1:9090/-/reload >/dev/null 2>&1 || systemctl reload prometheus 2>/dev/null || true
        ok "Prometheus scrapes Uptime Kuma as job=uptime-kuma"
      fi
    fi
  fi
fi
...
if [ -n "$GRAFANA_API_KEY" ]; then
  ...
  log "Syncing source dashboards and alert rules through the Monitoring API..."
  SYNC_BODY=$(mktemp)
  SYNC_STATUS=$(curl -s -o "$SYNC_BODY" -w "%{http_code}" -X POST "http://127.0.0.1:9099/api/v1/grafana/sync" \
    -H "X-Monitor-Key: ${API_KEY}" 2>/dev/null || echo "000")
  if [ "$SYNC_STATUS" -ge 200 ] 2>/dev/null && [ "$SYNC_STATUS" -lt 300 ] 2>/dev/null; then
    ok "Grafana dashboards and alert rules synced"
  else
    warn "Grafana sync endpoint returned ${SYNC_STATUS}: $(cat "$SYNC_BODY" 2>/dev/null)"
  fi
  rm -f "$SYNC_BODY"
else
...
[ -n "$UPTIME_KUMA_URL" ] && echo "  Uptime Kuma:    ${UPTIME_KUMA_URL} (Prometheus job: uptime-kuma)"
echo ""
