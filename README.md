...
- [Add Uptime Kuma Source](#add-uptime-kuma-source)
...
Uptime Kuma :3001/metrics ────────────► Prometheus job="uptime-kuma"
```
...
| Guide | Contents |
|-------|----------|
| [DOCKER_DEPLOYMENT.md](DOCKER_DEPLOYMENT.md) | Full Docker Compose stack — deploy, HTTPS, replication to new servers |
| [docs/GRAFANA_BRANDING_GUIDE.md](docs/GRAFANA_BRANDING_GUIDE.md) | Zentra logo, login theme, org name |
| [api/README.md](api/README.md) | Central Monitoring API — layout, env vars, local run |

## Dashboards

| Dashboard | Purpose |
|-----------|---------|
| **All Servers** | Server list, Uptime Kuma site list, Clients & Accounts org manager |
| **Uptime Kuma** | Uptime Kuma source health, monitor status, latency, availability |
...
sudo bash setup-server.sh \
  ...
  -uptime-kuma-url=http://UPTIME_KUMA_HOST:3001 \
  -uptime-kuma-key=UPTIME_KUMA_API_KEY
```
...
## Add Uptime Kuma Source

Uptime Kuma exposes Prometheus metrics at `/metrics`. Add it as a Prometheus
scrape source named `uptime-kuma`, then sync the bundled dashboard and alerts:

```bash
sudo bash scripts/add-uptime-kuma-source.sh \
  -url=http://UPTIME_KUMA_HOST:3001 \
  -api-key=UPTIME_KUMA_API_KEY
```

If Uptime Kuma metrics are not protected, omit `-api-key`. For new central
server installs, pass `-uptime-kuma-url` and `-uptime-kuma-key` to
`setup-server.sh`.

After Prometheus has scraped Kuma, open **All Servers → Uptime Kuma Sites** to
assign each site a Display Name, Client, and Account. Site alerts then route to
the same per-client recipients as server alerts.
...
├── scripts/
│   ├── deploy-docker-stack.sh      # Start/stop full Docker stack
│   ├── deploy-with-branding.sh     # Build branded Grafana + start stack
│   └── patch-alloy-port-probes.sh  # Fix per-port labels
├── dashboards/            # Main Org dashboard JSON (single source of truth)
...
region `ap-south-1`). Global alert rules live in the **Monitoring** folder
...
| Uptime Kuma Source Down | `up{job="uptime-kuma"} == 0` | 2m | critical |
| Uptime Kuma Monitor Down | `monitor_status{job="uptime-kuma"} == 0` | 2m | critical |
...
### Sync dashboards and alert rules

The API can re-sync source dashboards, alert rules, and notification routing into Grafana:

```bash
API_KEY=your_api_key
curl -X POST http://127.0.0.1:9099/api/v1/grafana/sync \
  -H "X-Monitor-Key: $API_KEY"
```

Custom main-org dashboards can be added as Grafana dashboard JSON payloads in
`dashboards/`. Custom alert rules can be added as Grafana provisioning
alert-rule JSON payloads in `api/app/alerts/`; use `__PROM_DS_UID__` for the
Prometheus datasource UID placeholder.
